...
func buildUpdateInput(args map[string]any, sctx contract.ServerContext) (protocol.UpdateJobInput, error) {
	...
	if executionMode != "" {
		...
		return protocol.UpdateJobInput{}, errors.New("reply_mode update requires execution_mode in the same call so the routing context stays consistent")
	}
	return input, nil
}
