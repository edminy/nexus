...
func (s *Service) GetDailyReport(ctx context.Context, input automationdomain.ScheduledTaskDailyReportInput) (*automationdomain.ScheduledTaskDailyReport, error) {
	...
	var jobs []automationdomain.ScheduledTask
	if jobID != "" {
		...
		if job == nil {
			...
			report := &automationdomain.ScheduledTaskDailyReport{
				Date:     date,
				Timezone: timezone,
				...
				StartAt:  startAt,
				EndAt:    endAt,
				Tasks:    []automationdomain.ScheduledTaskDailyReportItem{task},
			}
			addDailyReportTotals(&report.Totals, task.Totals)
			report.Totals.TaskCount = 1
			return report, nil
		}
		jobs = []automationdomain.ScheduledTask{*job}
	...
	report := &automationdomain.ScheduledTaskDailyReport{
		...
		StartAt:  startAt,
		EndAt:    endAt,
		Tasks:    make([]automationdomain.ScheduledTaskDailyReportItem, 0, len(jobs)),
	}
...
func (s *Service) buildDeletedDailyReportTask(
	...
	startAt time.Time,
	endAt time.Time,
) (automationdomain.ScheduledTaskDailyReportItem, error) {
	...
	if err != nil {
		return automationdomain.ScheduledTaskDailyReportItem{}, err
	}
	...
	if len(runs) == 0 && len(events) == 0 {
		return automationdomain.ScheduledTaskDailyReportItem{}, automationdomain.ErrJobNotFound
	}
	task := deletedDailyReportTaskFromEvents(normalizedJobID, events)
	for _, run := range runs {
		if !taskRunFallsInRange(run, startAt, endAt) {
			continue
		}
		run.DeliveryStatus = deriveTaskRunDeliveryStatus(run)
...
func deletedDailyReportTaskFromEvents(jobID string, events []automationdomain.ScheduledTaskEvent) automationdomain.ScheduledTaskDailyReportItem {
	task := automationdomain.ScheduledTaskDailyReportItem{
		JobID:   jobID,
		Name:    jobID,
		Deleted: true,
		Enabled: false,
...
func taskRunFallsInRange(run automationdomain.ScheduledTaskRun, startAt time.Time, endAt time.Time) bool {
...
func taskRunReportTime(run automationdomain.ScheduledTaskRun) time.Time {
...
func addDailyReportTotals(target *automationdomain.ScheduledTaskDailyReportTotals, source automationdomain.ScheduledTaskDailyReportTotals) {
