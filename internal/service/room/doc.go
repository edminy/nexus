...
//   - directed_message_* / message_causality.go / wake_timer_registry.go / public_*：定向消息、因果链、持久化唤醒队列、计时调度和公区 @ 唤醒。
...
//   - input_queue.go / input_queue_* / guidance_input.go：输入队列、round 终态确定性接力、引导送达确认、transport/错过 hook 回退、运行时补充上下文与已消费引导归组。
