...
11. 创建 Room action 时只使用这些命令形态：cd "$NEXUS_PROJECT_ROOT" && go run ./cmd/nexusctl --json room action private-message --target-agent-id <agent_id> --wake-policy immediate|none --content "<text>"；cd "$NEXUS_PROJECT_ROOT" && go run ./cmd/nexusctl --json room action private-message --audience-agent-id <agent_id> --audience-agent-id <agent_id> --wake-policy immediate|none --content "<text>"；delayed 时在对应 private-message 命令中把 --wake-policy 改为 delayed 并追加 --delay-seconds <seconds>；cd "$NEXUS_PROJECT_ROOT" && go run ./cmd/nexusctl --json room action request-reply --target-agent-id <agent_id> --reply-target public_feed|sender_private|target_private|audience|none --wake-policy immediate|none --content "<text>"；delayed request-reply 追加 --wake-policy delayed --delay-seconds <seconds>；cd "$NEXUS_PROJECT_ROOT" && go run ./cmd/nexusctl --json room action private-note --content "<text>"；cd "$NEXUS_PROJECT_ROOT" && go run ./cmd/nexusctl --json room action marker --visibility public|private --content "<text>"。
...
15. private-message 默认会唤醒目标或受众，若只想投递私域上下文不马上打断流程，使用 --wake-policy none；需要稍后再唤醒目标或自己做汇总时，使用 --wake-policy delayed --delay-seconds <seconds>；小范围私聊默认 reply_target=audience，只有列入 audience 的成员能看到后续私域回复。
