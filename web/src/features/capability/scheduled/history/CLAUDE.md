...
- `scheduled-task-run-history-model.ts` 只投影运行状态、时长和动作规则；`scheduled-task-run-diagnostic-model.ts` 独占诊断行、输出区块与复制文本定义。
- `scheduled-task-run-history-dialog.tsx` 只装配弹窗壳层和键盘关闭协议。
