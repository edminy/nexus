...
Nexus 内置 `nxs`（Nexus 原生）Agent runtime，默认即用，后端机器**无需安装 Claude Code**。原生 runtime 直连 LLM API（支持 Anthropic Messages 与 OpenAI Chat Completions 协议），通过环境变量 `NEXUSCTL_COMMAND_PATH` 指向项目自带的 `nexusctl` 二进制即可。

仅当你需要让 Agent 运行在真实 Claude Code 环境中（使用其完整 skill / hook / 工具链）时，才需安装 Claude Code：将 Agent 的 runtime 切换为 `claude`，并确保 `claude` 在后端机器的 `PATH` 中可用。
