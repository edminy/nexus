...
工具成功后只发送一条简短最终回复，然后停止并等待用户输入；不要继续调用工具或开启新工作。最终回复应明确说当前 Goal 已完成、可以清理，不要描述成暂停；简短总结完成了什么，并按结果里的 `goal.tokensUsed`、`goal.tokenBudget` 和 `goal.timeUsedSeconds` 报告最终用量与耗时。
