...
公区 final reply 中的非代码 `@成员` 表示真实 handoff。源 Agent 的 final reply 持久化且 source slot 成功收口后立即处理，不等待同一 root round 的其他 slot；解析使用成员 name、display name 或 agent id，反引号代码区域中的 `@` 不触发唤醒。目标重复时只唤醒一次，不能唤醒自己。

用户消息里的 `@成员` 只负责解析输入目标；Agent final reply 里的 `@成员` 才创建 Agent-to-Agent public handoff。多个不同目标按目标拆成多个独立 handoff，目标的书写顺序只决定创建顺序，不承诺回复顺序。
...
- 目标正在运行：按输入的 delivery policy 选择 guide、queue 或 interrupt；guide 没有可靠 applied ACK 时降级为 queue。
