...
- Shell 默认注册 `Option + Space` 全局快捷键，可从系统任意位置唤起独立 launcher 浮层；浮层支持失焦隐藏和 Escape 关闭。
...
scripts/desktop/package-macos-dogfood.sh
```
...
没有 Developer ID 时，当前发布链路只面向内部 dogfood，不作为公开 macOS 发布包：

```bash
scripts/desktop/package-macos-dogfood.sh
```

默认输出到 `desktop/macos/.build/dogfood/`：

- `Nexus-macos-<version>-<build>.zip`
- `Nexus-macos-<version>-<build>.zip.sha256`
- `Nexus-macos-<version>-<build>.metadata.json`

安装前先校验 sha256：

```bash
cd desktop/macos/.build/dogfood
shasum -a 256 -c Nexus-macos-<version>-<build>.zip.sha256
```

解压后把 `Nexus.app` 拖到 `/Applications`。因为 dogfood 包是 ad-hoc 签名且未公证，macOS 可能拦截首次打开；可信内部测试包优先用 Finder 右键 Open。仅本地测试机器可在校验 sha256 后清理 quarantine：

```bash
xattr -dr com.apple.quarantine /Applications/Nexus.app
```

卸载或重置 dogfood 数据时，先退出 Nexus，再按需要删除：
...
- 还没有 Developer ID 签名、公证和自动更新；当前 dogfood 包是 ad-hoc 签名。
